...
function MessengerCard() {
  ...
  /*
   *  ⛔ نبضِ پنج‌ثانیه‌ای رفت — بندِ ۱.۵-الف تمام شد. دفترِ پیام‌رسان از
   *  ۱.۵۰.۰ به گذرگاه وصل است (`messenger/index.js` ⇒ `touched()`)، پس
   *  هر کدِ تازه همان لحظه این‌جا می‌آید و تا چیزی عوض نشود **صفر**
   *  درخواست می‌رود.
   *
   *  ⚠️ کفِ شصت‌ثانیه‌ای می‌ماند و دلیلش انقضاست، نه نوشتن: کدها بعد از
   *  ده دقیقه خودبه‌خود از فهرست می‌افتند و آن **هیچ نوشتنی ندارد که
   *  خبر بدهد**. شصت ثانیه به‌جای پنج یعنی دوازده برابر کمتر.
   */
  useLive('messenger', load, 60000);
...
function NotifyCard({ addresses }: { addresses: SiteServerInfo['addresses'] }) {
  ...
  /*
   *  ⛔ و همان‌طور نبضِ شش‌ثانیه‌ایِ اعلان‌ها — `notify/index.js` حالا
   *  با هر موضوعِ تازه، رمزِ چرخیده، پیامِ تازه و دستگاهِ آمده/رفته خبر
   *  می‌دهد. ⚠️ کف این‌جا هم برای شمارِ شنونده‌های زنده است که با قطع و
   *  وصلِ وب‌سوکت عوض می‌شود و نوشتنی ندارد.
   */
  useLive('notify', load, 60000);
