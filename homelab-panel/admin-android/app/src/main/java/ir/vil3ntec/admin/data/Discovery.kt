...
data class FoundServer(
   ...
   *
   *  ⚠️ به کلید بستگی دارد، و این نکته یک بار کلِ ورود را خواباند.
   *
   *  آدرسِ admin.<دامنه> پشتِ درِ کلیددار است: بدونِ کلید، *هر* مسیرش —
   *  حتی /health و حتی خودِ ورود — «not found» می‌دهد. این عمدی است و
   *  درست، ولی یعنی تا کلید نداریم آن آدرس به هیچ دردی نمی‌خورد.
   *
   *  وقتی سرور در همین شبکه پیدا شده، یعنی همین‌جاییم — پس آدرسِ خانه
   *  می‌نشیند: مستقیم است، سریع است، و از اینترنت رد نمی‌شود. کلیدِ در هم
   *  همان لحظهٔ ورود صادر می‌شود، پس دفعهٔ بعد که بیرون باشیم آماده است:
   *
   *      در خانه  →  آدرسِ خانه  →  ورود  →  کلید صادر می‌شود
   *      بیرون    →  admin.<دامنه> با همان نام و رمز، هر جای دنیا
   *
   *  ⚠️ بیرونِ خانه چیزی پیدا نمی‌شود و این تابع اصلاً صدا زده نمی‌شود؛
   *  آن‌جا کاربر خودِ آدرسِ دامنه را می‌نویسد و صفحهٔ ورود کلید را می‌گیرد.
   *
   *  ⚠️ آدرسِ تونل (مثلِ sync.example.com) هیچ‌وقت گزینه نیست: به پورتِ
   *  عمومی می‌رود و پنل آن‌جا اصلاً سرو نمی‌شود.
   */
  fun best(hasKey: Boolean): String = if (hasKey && admin.isNotBlank()) admin else url
