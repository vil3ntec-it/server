...
//  ولی تا روشن نشود هیچ برنامه‌ای از راهِ api.<دامنه> وارد نمی‌شود.
{
  ...
  if (raw === '0') {
    ...
    try {
      ...
      warn(`سرورِ حساب روشن نیست (${url}) — برنامه‌ها تا روشن شدنش وارد نمی‌شوند. پنل خودش بالا می‌آوردش (پوشهٔ account-server کنارِ پنل، یا ریپوی shop/server کنارِ این ریپو)؛ پس از بالا آمدنِ پنل، «پنل ← پمپ‌ها ← تنظیمات و داده‌ها ← سرورِ حساب» را ببینید.`);
    } finally {
      clearTimeout(timer);
    }
  }
