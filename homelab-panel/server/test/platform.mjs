...
async function api(method, url, body, opts = {}) {
  ...
  // APIِ نسخه‌دارِ پنل زیرِ /api/panel/v1 نشسته، چون /api/v1 مالِ APIِ برنامهٔ
  // توحید است (اپ‌های بیرون‌آمده همان را صدا می‌زنند).
  const res = await fetch(BASE + url.replace(/^\/api\/v1(?=\/|$)/, '/api/panel/v1'), { method, headers, body: payload });
...
async function main() {
  ...
  check('مسیرِ بی‌پیشوندِ /api هنوز کار می‌کند', r.status === 200);
  // /api پس از ادغام همچنان مسیرِ زندهٔ پنل است (رابط کاربری همان را صدا
  // می‌زند)، نه یک نامِ مستعارِ منسوخ — پس نباید هدرِ Deprecation بگیرد.
  check('/api منسوخ اعلام نمی‌شود', !r.headers.get('deprecation'));
