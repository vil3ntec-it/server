...
//
//  ⚠️ خودِ طرح از فایلی است که صاحبِ سرور داد و عمداً دست‌نخورده مانده:
//  همان رنگ‌ها، همان چیدمان، همان متن‌ها. چیزی که این‌جا اضافه شده فقط
//  پر کردنِ جاهای خالی است و آن هم با احتیاط.
//
//  ⚠️ چرا HTMLِ ایمیل این‌قدر عجیب نوشته می‌شود:
//
//    • چیدمان با <table> است، نه flex و نه grid. Outlook روی ویندوز با
//      موتورِ Word رندر می‌کند و آن دو را اصلاً نمی‌فهمد.
//    • هر جا گرادیان هست، یک bgcolor ساده هم کنارش گذاشته شده — کلاینتی
//      که گرادیان را نمی‌فهمد، رنگِ تخت می‌بیند نه سفیدی.
//    • هیچ جاوااسکریپتی نیست؛ هیچ کلاینتِ ایمیلی اجازه‌اش را نمی‌دهد.
//    • هیچ تصویری از بیرون بار نمی‌شود — آیکنِ پاکت با خودِ CSS کشیده
//      شده. بیشترِ کلاینت‌ها تصویرها را تا اجازهٔ کاربر نمی‌آورند و
//      ایمیل نصفه دیده می‌شود.
// ---------------------------------------------------------------------------

/** متنی که از بیرون می‌آید نباید بتواند از تگ بیرون بزند */
function esc(value) {
  return String(value ?? '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
 ...
 *  آدرسِ امن برای href.
 *
 *  ⚠️ لازم است: اگر آدرس از تنظیمات بیاید و کسی `javascript:` بگذارد،
 *  در بعضی کلاینت‌های قدیمی اجرا می‌شود. فقط http و https رد می‌شوند.
 */
 ...
 * @param {string} [o.appName]       نامی که در عنوانِ ایمیل می‌نشیند
 * @param {string} [o.name]          نامِ گیرنده — اگر ندهی، خوش‌آمدِ ساده
 * @param {string} [o.actionUrl]     مقصدِ دکمه — اگر ندهی، دکمه نمی‌آید
 * @param {string} [o.siteUrl]       آدرسِ سایت در پانویس
...
export function otpEmail({
  ...
  /*
   *  ⚠️ «فلانی عزیز، به VILL3N خوش آمدید» فقط وقتی معنی دارد که نامی
   *  داشته باشیم. بی نام، «عزیز» تنها می‌ماند و بد دیده می‌شود.
   */
  const who = esc(String(name || '').trim());
  const greeting = who ? `${who} عزیز، به VILL3N خوش آمدید` : 'به VILL3N خوش آمدید';

  /*
   *  ⚠️ دکمه فقط با آدرسِ واقعی می‌آید. دکمه‌ای که به هیچ‌جا نمی‌رود، از
   *  نبودنش بدتر است — کاربر می‌زندش و هیچ اتفاقی نمی‌افتد.
   */
  const verify = safeUrl(actionUrl);
  const button = verify
...
`
  ...
  const site = safeUrl(siteUrl) || 'https://vill3n.top';
...
${button}      </td></tr>
  ...
  <tr><td class="px" align="center" style="padding:30px 40px 40px;font-family:'Vazirmatn',Tahoma,Arial,sans-serif;font-size:12px;line-height:2;color:#7a6a82;">
    ...
    <div style="margin-top:10px;">
      <a href="${site}" style="color:#c2185b;font-weight:700;">VILL3N</a> &copy; ${year} | <a href="mailto:vil3ntec@gmail.com" style="color:#c2185b;">vil3ntec@gmail.com</a>
    </div>
  </td></tr>
  ...
  const text = [
    'VILL3N',
    '',
    who ? `${name} عزیز، خوش آمدید.` : 'خوش آمدید.',
    '',
    `کد ورود شما: ${String(code ?? '')}`,
    `این کد تا ${mins} دقیقه معتبر است.`,
    verify ? `\nتأیید حساب: ${actionUrl}` : '',
    '',
    'این کد را در اختیار هیچ‌کس قرار ندهید؛ VILL3N هرگز کد تأیید را از شما درخواست نمی‌کند.',
    'اگر این درخواست از طرف شما نبوده است، این پیام را نادیده بگیرید.',
    '',
    safeApp === 'VILL3N' ? '' : appName,
