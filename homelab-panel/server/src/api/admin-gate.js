...
/*
 *  کدام درخواستِ ردشده واقعاً یک «حدس» است.
 *
 *  ⛔ یک کلیدِ غلط، هرچند بار تکرار شود، *یک* حدس است — و این قاعده
 *  بی‌اهمیت به نظر می‌رسد تا روزی که برنامه را از کار می‌اندازد:
 *
 *  گوشی‌ای که کلیدش باطل شده (یا پنلی که از نو نصب شده و دفترِ کلیدهایش
 *  خالی است) همان کلیدِ مرده را هر شش ثانیه دوباره می‌زند، چون چراغِ
 *  وضعیتِ صفحهٔ ورود نبض دارد. بیست ۴۰۴ در دو دقیقه یعنی سقفِ در؛ و چون
 *  نبض هرگز نمی‌ایستد، پنجرهٔ ده‌دقیقه‌ای هیچ‌وقت خالی نمی‌شود. از آن به
 *  بعد حتی کلیدِ **تازه و درست** هم ۴۲۹ می‌گیرد: برنامه خودش را برای
 *  همیشه بیرون گذاشته، در حالی که سرور سالم است و آدرس هم درست.
 *  (همان «برنامهٔ ادمین کار نمی‌کند» با سرورِ روشن.)
 *
 *  پس شمارش به «کلیدِ تازه‌ای که تا حالا از این IP ندیده‌ایم» بسته شد.
 *  حدس‌زننده مجبور است کلید را عوض کند تا جلو برود، پس دفاع مو‌به‌مو سرِ
 *  جایش است — سنجه‌اش «حدس‌های متفاوت همچنان قفل می‌شوند» است و اگر این
 *  را ضعیف کنید همان‌جا قرمز می‌شود.
 *
 *  ⚠️ درخواستِ **بی کلید** اصلاً حدس نیست و شمرده نمی‌شود: نبضِ برنامه
 *  پیش از ورود، و هر کسی که آدرس را باز می‌کند، همین‌اند.
 */
const guesses = new Map(); // "ip|hash" → آخرین باری که دیده شد
const GUESS_TTL = 10 * 60 * 1000;

function pruneGuesses(now) {
  if (guesses.size < 5_000) return;
  for (const [k, at] of guesses) if (now - at > GUESS_TTL) guesses.delete(k);
  // اگر باز هم بزرگ بود، کهنه‌ترین‌ها می‌روند — حافظه سقف دارد
  if (guesses.size >= 5_000) {
    for (const k of [...guesses.keys()].slice(0, 1_000)) guesses.delete(k);
  }
}

export function gateGuessIsNew(req, res) {
  if (res.statusCode < 400) return false;

  const key = String(req.headers[GATE_HEADER] || '').trim();
  if (!key) return false; // بی کلید یعنی حدسی در کار نیست

  const now = Date.now();
  pruneGuesses(now);

  const seenKey = `${clientIp(req)}|${hash(key)}`;
  const last = guesses.get(seenKey);
  guesses.set(seenKey, now);
  return !(last && now - last <= GUESS_TTL);
}

/** فقط برای آزمون‌ها — حافظهٔ حدس‌ها را خالی می‌کند */
export function resetGateGuesses() {
  guesses.clear();
}
