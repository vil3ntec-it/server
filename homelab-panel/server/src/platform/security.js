...
// ---------------------------------------------------------------------------
//  IP واقعیِ درخواست — تنها جای این محاسبه در کلِ سرور
//
...
//  بسازد تا محدودیتِ نرخ را دور بزند.
//
//  ⚠️ و این حرف نبود، اندازه گرفته شد. با نسخهٔ قبلی که هدر را بی‌قید‌وشرط
//  می‌خواند:
//
//      ۲۵ تلاشِ ورود از یک IP           → ۱۵ تا مسدود شد
//      ۲۵ تلاش با X-Forwarded-Forِ جعلی → ۰ تا مسدود شد
//
//  یعنی سقفِ ورود، سقفِ کد و سقفِ API هر سه با یک هدرِ ساده بی‌اثر می‌شدند.
//
//  پس هدر فقط در دو حالت باور می‌شود:
//
//    ۱) HLP_TRUST_PROXY=1 — خودتان گفته‌اید پراکسی جلو هست
//    ۲) خودِ اتصال از لوکال‌هاست آمده باشد
//
//  حالتِ دوم همان چیزی است که نبودش این سوراخ را ساخته بود: با تونلِ
//  cloudflared، هر درخواست از 127.0.0.1 می‌رسد. بی این استثنا، *همهٔ*
//  کاربرانِ اینترنتی در یک سطل می‌افتادند و یک نفر می‌توانست بقیه را قفل
//  کند. و امن است، چون بسته‌ای که واقعاً از اینترنت می‌آید هیچ‌وقت نشانیِ
//  مبدأش لوکال‌هاست نیست — پس مهاجمِ بیرونی نمی‌تواند وارد این حالت شود.
//
//  ترتیبِ خواندن: CF-Connecting-IP اول، چون کلودفلر همیشه آن را خودش
//  می‌نویسد و هر چه کاربر فرستاده باشد را دور می‌ریزد؛ X-Forwarded-For
//  را می‌شود با زنجیرهٔ جعلی آلود.
// ---------------------------------------------------------------------------

const LOOPBACK = /^(::1|::ffff:127\.|127\.)/;

/** آیا این اتصال از خودِ همین دستگاه آمده؟ (تونل، پراکسیِ محلی) */
function fromLoopback(req) {
  return LOOPBACK.test(String(req.socket?.remoteAddress || ''));
}

export function clientIp(req) {
  if (config.trustProxy || fromLoopback(req)) {
